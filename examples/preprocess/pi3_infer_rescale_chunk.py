...
def to_numpy(tensor: torch.Tensor | np.ndarray) -> np.ndarray:
    """
    Convert a torch.Tensor into a NumPy array, or return it directly if already numpy.
    Automatically handles device transfers and detaches gradients.
    """
...
def create_tsdf_mesh(
    ...
    # Estimate the scene scale (based on camera poses)
    ...
    if len(cam_centers) > 1:
        ...
        med_dist = 1.0
    # Default voxel size is median distance / 256 (adjust if needed)
    ...
    for i in range(N):
        ...
        # Depth shapes may differ from (h, w); try resizing when that happens
        ...
        # Open3D expects float depth images consistent with depth_scale; we use meters with scale 1.0
        ...
        rgbd = o3d.geometry.RGBDImage.create_from_color_and_depth(
            ...
            depth_trunc=1000.0,  # loose truncation; sdf_trunc governs TSDF
            convert_rgb_to_intensity=False,
        )

        # Construct Open3D intrinsics
        K = ixts[i]
        fx = float(K[0, 0])
        fy = float(K[1, 1])
        cx = float(K[0, 2])
        ...
        try:
            ...
            )  # NOTE: Open3D historically consumes world-to-camera matrices
            # Explanation:
            ...
            # Fall back by swapping the extrinsic convention
            try:
    ...
    # ======= Remove floating small mesh components =======
...
def main():
    ...
    # Extract metadata from COLMAP folders
    ...
    if skip_inference:
        print("[INFO] Skipping inference; using existing chunk_predictions")
    else:
    ...
    for chunk_idx, fname in enumerate(chunk_files):
        ...
        # Parse global start/end from filename (predictions_{start:05d}_{end-1:05d}.npz)
        ...
        for i, E in enumerate(predictions["camera_poses"]):
            R_cw = E[:3, :3]  # 3×3 rotation
            t_cw = E[:3, 3]  # translation
            R_wc = R_cw.T
            t_wc = -R_cw.T @ t_cw
        ...
        # Resize + stage1 rescale: ensure per-frame hws match the global values
        depths_resized = []
        ...
        # Stage 2: refine each frame with sparse COLMAP projections
        for i_local in range(len(predictions["depth_map"])):
            ...
            print(
                f"[Global Frame {global_idx+1:05d}] fit result: a = {a:.6f}, b = {b:.6f}, inliers = {inlier_count}/{len(d_pi3)}"
            )
            ...
            # d_min, d_max = np.percentile(valid_depths, [1, 99])  # remove extremes
        ...
        # ====== 4.1 Align poses and rescale depth end ========
        ...
        for i_local in range(len(predictions["depth_map"])):
            ...
            # Resize confidence to the original resolution
            ...
            # Keep a float32 copy for TSDF fusion
            all_depths_resized.append(resized_depth.copy())

            # Store depth on disk as float16 if possible
            ...
            if args.save_depth_vis:
                ...
                depth_normalized = cv2.normalize(
                    depth_vis_float, None, 0, 255, cv2.NORM_MINMAX
                )
                ...
                cv2.imwrite(
                    depth_path,
                    depth_colored,
                    [int(cv2.IMWRITE_JPEG_QUALITY), 90],  # quality 0-100
                )
    ...
    # Prefer mesh vertex colors; otherwise fall back to grayscale
    ...
    if mesh.has_vertex_colors() and np.asarray(mesh.vertex_colors).size != 0:
        ...
        print(
            "[WARN] mesh has no vertex colors; using default gray for point cloud export."
        )

    # Construct an Open3D point cloud and save as colored PLY
