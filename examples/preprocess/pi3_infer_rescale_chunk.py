...
def main():
        ...
        with torch.no_grad():
            with torch.amp.autocast('cuda', dtype=dtype):
                for start in range(0, len(image_paths), chunk_size):
                    ...
                    # imgs shape: (N, 3, H, W).
                    # imgs value: [0, 1]
                    batch_images = load_images_path_as_tensor(batch_paths, interval=1).to(device)
    ...
    for chunk_idx, fname in enumerate(chunk_files):
        ...
        for i_local in range(len(predictions['depth_map'])):
            ...
            print(f"[Global Frame {global_idx+1:05d}] 拟合结果: a = {a:.6f}, b = {b:.6f}, 内点数 = {inlier_count}/{len(d_pi3)}")
        ...
        for i_local in range(len(predictions['depth_map'])):
            ...
            resized_depth = resized_depth.astype(np.float32, copy=True)
            ...
            valid_conf_mask = (resized_depth_conf >= conf_threshold) & (resized_depth_conf > 1e-5)

            invalid_mask = ~valid_conf_mask
            resized_depth[invalid_mask] = 0.0

            # 将清理后的深度保存给 TSDF（保持 float32 精度）
            all_depths_resized.append(resized_depth.copy())

            # 检查并转换数据类型为float16用于存盘
            depth_to_save = resized_depth
            if depth_to_save.dtype != np.float16:
                valid_depths = depth_to_save[depth_to_save > 0]
            ...
            np.save(depth_npy_path, depth_to_save)

            # 保存可视化 .jpeg 
            if args.save_depth_vis:
                depth_vis_float = resized_depth.astype(np.float32)
                # 标准化到 0-255
                depth_normalized = cv2.normalize(depth_vis_float, None, 0, 255, cv2.NORM_MINMAX)
                depth_normalized = depth_normalized.astype(np.uint8)
                depth_colored = cv2.applyColorMap(depth_normalized, cv2.COLORMAP_JET)

                zero_mask = (depth_vis_float == 0)
                depth_colored[zero_mask] = (255, 255, 255)

                # 保存为JPEG（调整质量参数）
                depth_path = os.path.join(depth_vis_dir, f"depth_{global_idx+1:05d}.jpg")
                cv2.imwrite(
                    depth_path, 
                    depth_colored,
    ...
    if mesh.has_vertex_colors() and np.asarray(mesh.vertex_colors).size != 0:
        ...
        print("[WARN] mesh has no vertex colors; using default gray for point cloud export.")
