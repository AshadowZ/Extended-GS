...
def crop_vggt_pred_to_original_dimensions(vggt_pred, original_width, original_height, target_size=518):
    ...
    if len(vggt_pred.shape) == 3:  # HxWx3
        cropped_pred = vggt_pred[pad_top:pad_top + new_height, 
                                    pad_left:pad_left + new_width, :]
    else:  # HxW
        cropped_pred = vggt_pred[pad_top:pad_top + new_height, 
                                    pad_left:pad_left + new_width]
    
    return cv2.resize(cropped_pred, (original_width, original_height), interpolation=cv2.INTER_NEAREST)
...
def create_tsdf_mesh(depths_resized, ixts, camtoworlds, image_paths, hws, 
                     voxel_length=None, sdf_trunc=None, color_type=o3d.pipelines.integration.TSDFVolumeColorType.RGB8):
    """
    ...
    print(f"[TSDF] N={N}, med_camera_dist={med_dist:.3f} m, voxel_length={voxel_length:.6f} m, sdf_trunc={sdf_trunc:.6f} m")
    ...
    for i in range(N):
        ...
        # integrate
        try:
            tsdf_vol.integrate(rgbd, intrinsic, np.linalg.inv(extrinsic))  # NOTE: Open3D 的 integrate 接受 extrinsic = world_to_camera ? see below
            # Explanation:
...
def main():
    ...
    parser.add_argument("--tsdf_frame_interval", type=int, default=1, 
    ...
    ixts, exts, hws, point_ids, names = read_ixt_ext_hw_pointid(cams, images, colmap_points)
    ...
    if missing:
        print(f"[WARN] {len(missing)} images not found in {image_dir}, e.g. {missing[:3]}")
    ...
    if first_height > first_width:
        ...
        print(f"[INFO] 第一张图像 {first_width}x{first_height} (H>W)，使用 pad 模式")
    else:
        # 宽度 >= 高度，使用 crop 模式
        preprocessing_mode = "crop"
        print(f"[INFO] 第一张图像 {first_width}x{first_height} (W>=H)，使用 crop 模式")
    first_image.close()
    ## ====== 1. 读取 COLMAP 元数据 end ========
    ...
    state_dict = torch.load(_URL, map_location='cpu') # 代码有屎
            ...
            with torch.cuda.amp.autocast(dtype=dtype):
                for start in range(0, len(image_paths), chunk_size):
                    ...
                    if preprocessing_mode == "pad":
                        batch_images = load_and_preprocess_images(batch_paths, mode="pad").to(device)
                    else:
                        batch_images = load_and_preprocess_images(batch_paths, mode="crop").to(device)

                    print(f"[Chunk {start}–{end-1}] Inference ...")
                    predictions = model(batch_images)

                    # 处理输出
                    extrinsic, intrinsic = pose_encoding_to_extri_intri(predictions["pose_enc"], batch_images.shape[-2:])
                    ...
                    # 删除不需要的字段
                    for k in ["pose_enc", "pose_enc_list", "world_points", "world_points_conf", "images"]:
                        if k in predictions:
                            del predictions[k]
                    ...
                    # 保存
                    out_file = os.path.join(chunk_dir, f"predictions_{start:05d}_{end-1:05d}.npz")
    ...
    for chunk_idx, fname in enumerate(chunk_files):
        ...
        for i_local in range(len(predictions['depth'])):
            ...
            # 根据预处理模式选择不同的恢复方式
            if preprocessing_mode == "pad":
                # pad模式：使用crop_vggt_pred_to_original_dimensions恢复
                depth_resized = crop_vggt_pred_to_original_dimensions(depth, original_width, original_height)
            else:
                # crop模式：直接使用cv2.resize INTER_NEAREST恢复
                depth_resized = cv2.resize(depth, (original_width, original_height), interpolation=cv2.INTER_NEAREST)
        ...
        for i_local in range(len(predictions['depth'])):
            ...
            if np.count_nonzero(valid_mask) < 10:
                print(f"[WARN] Global Frame {global_idx+1} has too few valid sparse points.")
                continue
            ...
            print(f"[Global Frame {global_idx+1:05d}] 拟合结果: a = {a:.6f}, b = {b:.6f}, 内点数 = {inlier_count}/{len(d_vggt)}")
        ...
        for i_local in range(len(predictions['depth'])):
            ...
            if preprocessing_mode == "pad":
                ...
                resized_depth_conf = cv2.resize(depth_conf, (original_width, original_height), interpolation=cv2.INTER_NEAREST)
            ...
            # 深度边缘检测与过滤
            depth_tensor = torch.from_numpy(resized_depth).float().unsqueeze(0).unsqueeze(0).cuda()
            ...
            # 保存可视化 .png 
            if args.save_depth_vis:
                depth_normalized = cv2.normalize(resized_depth, None, 0, 255, cv2.NORM_MINMAX)
                ...
                depth_path = os.path.join(depth_vis_dir, f"depth_{global_idx+1:05d}.png")
        ...
        if args.save_point_clouds:
            ...
            for i_local in range(len(predictions['depth'])):
                ...
                original_height, original_width = hws[global_idx]
                u, v = np.meshgrid(np.arange(original_width), np.arange(original_height))
                ...
                output_path = os.path.join(output_dir, f"point_cloud_{global_idx+1:05d}.ply")
    ...
    if tsdf_frame_interval > 1:
        ...
        print(f"[TSDF] 使用间隔 {tsdf_frame_interval}，从 {total_frames} 帧中选择 {len(selected_indices)} 帧进行重建")
        # 使用部分帧
    ...
    mesh = create_tsdf_mesh(selected_depths, selected_ixts, selected_camtoworlds, selected_image_paths, selected_hws,
